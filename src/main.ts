...
function saveFavoriteCities(
    favorites: { name: string; lat: number; lon: number; state?: string }[]
) {
    try {
...
export async function getWeatherData(
    lat: number,
    lon: number,
    locationData: { lat: number; lon: number; name: string; state?: string },
    ...
    try {
        ...
        if (data.current) {
            displayCurrentWeather(
                uiElements.currentWeatherDisplay,
                data.current,
                preferredUnits
            );
        } else {
        ...
        if (data.alerts) {
            displayAlerts(
                uiElements.alertsDisplay,
                uiElements.activeAlertsLink,
                data.alerts
            );
        } else {
        ...
        displayError(
            uiElements.errorMessageDisplay,
...
export function displayCurrentWeather(
    ...
    currentWeatherDisplay.innerHTML = `
        ...
        <p>Temperature: ${Math.round(
            current.temp
        )}${tempUnitSymbol} (Feels like: ${Math.round(
        current.feels_like
    )}${tempUnitSymbol})</p>
        <p>Condition: ${
            current.weather[0].description.charAt(0).toUpperCase() +
            current.weather[0].description.slice(1)
        } <br><i class="wi ${iconClass}"></i></p>
        <p>Humidity: ${current.humidity}%</p>
        <p>Wind Speed: ${current.wind_speed} ${
        preferredUnits === "metric" ? "m/s" : "mph"
    }</p>
        <p>Sunrise: ${new Date(current.sunrise * 1000).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
        })}</p>
        <p>Sunset: ${new Date(current.sunset * 1000).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
        })}</p>
...
export async function getLatLonFromCity(
    input: string,
    uiElements: UIElements
): Promise<{ lat: number; lon: number; name: string; state?: string } | null> {
    let geoUrl: string;
        ...
        if (/^\d{5}$/.test(input)) {
            ...
            if (data.lat && data.lon) {
                ...
                if (!reverseResponse.ok) {
                    ...
                    console.error(
                        `HTTP error on reverse geocoding! status: ${reverseResponse.status}`
                    );
                    showLoading(uiElements.loadingOverlay, false);
                ...
                const state =
                    reverseData.length > 0 ? reverseData[0].state : undefined;
                showLoading(uiElements.loadingOverlay, false);
                return {
                    lat: data.lat,
                    lon: data.lon,
                ...
                displayError(
                    uiElements.errorMessageDisplay,
                    `Zip code "${input}" not found. Please try again.`
                );
            ...
            if (data.length > 0) {
                ...
                displayError(
                    uiElements.errorMessageDisplay,
                    `City "${input}" not found. Please try adding state/country (e.g., "Wilmington, NC, US" or "London, UK").`
                );
                uiElements.cityDisplayName.textContent = "";
                uiElements.cityDisplayName.style.display = "none";
                showLoading(uiElements.loadingOverlay, false);
...
export function initializeUI(): UIElements {
    ...
    const currentWeatherDisplay = document.getElementById(
        "current-weather-display"
    ) as HTMLDivElement;
    const forecastDisplay = document.getElementById(
        "forecast-display"
    ) as HTMLDivElement;
    const alertsDisplay = document.getElementById(
        "alerts-display"
    ) as HTMLDivElement;
    const cityDisplayName = document.getElementById(
        "city-name-display"
    ) as HTMLDivElement;
    const locationNameSpan = document.getElementById(
        "location-name"
    ) as HTMLSpanElement;
    const activeAlertsLink = document.getElementById(
        "active-alerts-link"
    ...
    const favoriteStar = document.getElementById(
        "favorite-star"
    ) as HTMLSpanElement;
    ...
    unitToggle.addEventListener("change", async (event) => {
        ...
        if (target.name === "temp-unit") {
            ...
            if (lastLocation) {
                ...
                uiElements.locationNameSpan.textContent = lastLocation.state
                    ? `${lastLocation.name}, ${lastLocation.state}`
                    : lastLocation.name;
...
async function performSearch(uiElements: UIElements) {
    ...
    if (city) {
        ...
        if (coords) {
            ...
            uiElements.locationNameSpan.textContent = coords.state
                ? `${coords.name}, ${coords.state}`
                : coords.name;
            uiElements.cityDisplayName.style.display = "block";
        ...
        displayError(
            uiElements.errorMessageDisplay,
            'Please enter a city name or use "Use My Location".'
        );
        uiElements.currentWeatherDisplay.innerHTML = "";
            ...
            async (position) => {
                ...
                if (reverseResponse.ok) {
                    ...
                    if (reverseData.length > 0) {
                        ...
                        uiElements.locationNameSpan.textContent = location.state
                        ...
                        await getWeatherData(
                            lat,
                            lon,
                            { lat: lat, lon: lon, name: "Unknown Location" },
                            uiElements
                        );
                    }
                } else {
            ...
            (error) => {
                ...
                switch (error.code) {
                    case error.PERMISSION_DENIED:
                        errorMessage =
                            "Location access denied. Please enable location services for this site in your browser settings.";
                        break;
